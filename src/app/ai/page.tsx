  ...
  useEffect(() => {
    ...
    async function loadFeed() {
      ...
      try {
        ...
        const result = await api.getInsightFeed(token, { tab, limit: 12 });
        if (!result.success) {
          if (!cancelled) {
            setFeedState('error');
      ...
      <section className="ct-section">
        ...
        <div className="insight-chip-row">
          ...
          {keywords.map((keyword) => (
            <span key={keyword} className="insight-chip">{keyword}</span>
          ))}
        </div>
      </section>
            ...
            {feedItems.map((item) => (
              <article key={item.id} className="insight-card">
                ...
                <div className="insight-card-actions">
                  ...
                  <a className="insight-action-btn insight-action-btn--link" href={item.sourceUrl} target="_blank" rel="noreferrer">
                    원문 보기
                  </a>
                </div>
          ...
          <div className="insight-saved-list">
            {savedItems.map((item) => (
              <article key={item.id} className="insight-saved-card">
                ...
                <div className="insight-chip-row">
                  {item.linkedJob ? <span className="insight-chip">{item.linkedJob}</span> : null}
                  {item.linkedMajor ? <span className="insight-chip">{item.linkedMajor}</span> : null}
                  {(item.tags ?? []).slice(0, 3).map((tag) => (
                    <span key={`${item.id}-${tag}`} className="insight-chip insight-chip--muted">{tag}</span>
                  ))}
                </div>
                <div className="insight-card-actions">
                  <a className="insight-action-btn insight-action-btn--link" href={item.sourceUrlSnapshot} target="_blank" rel="noreferrer">
                    원문 보기
                  </a>
                  <button
                    type="button"
