...
핵심 컬렉션: `users/{uid}`, `users/{uid}/testSessions/{sessionId}`, `users/{uid}/testResults/{testId}`, `users/{uid}/records/{recordId}`, `majors/{majorId}`, `subjects/{subjectId}`

상세 스키마: `docs/firestore-schema-v1.0.md` 참조
...
- Path alias: `@/*` → `./src/*`
- 색상은 반드시 CSS 변수(`--brand-700`, `--ink` 등) 사용. hex 하드코딩 금지. 테마 상세: `docs/design-theme-v1.0.md`

### Firestore 개발 규칙

- **서브컬렉션 추가 시**: `firestore.rules`에 보안 규칙 명시, `firestore.indexes.json`에 복합 인덱스 추가, `docs/firestore-schema-v1.0.md`에 스키마 기록
- **Upsert 패턴**: 조회 후 조건부 생성/업데이트는 반드시 `runTransaction()`으로 감싸서 race condition 방지
- **외부 API 시간값**: 외부 API가 요구하는 포맷 필드와 서버 타임스탬프 필드를 둘 다 저장
- **비동기 업데이트**: 백그라운드 fetch 시 `{fieldName}Fetching` 플래그로 중복 처리 방지, 성공/실패 모두 `FieldValue.delete()`로 플래그 해제
...
- `docs/firestore-schema-v1.0.md` - Firestore 데이터베이스 스키마 (컬렉션, 필드, 보안규칙, 인덱스, 개발 규칙)
- `docs/design-theme-v1.0.md` - 디자인 테마 가이드 (Midnight Pine 컬러 팔레트, CSS 변수, 사용 규칙)
